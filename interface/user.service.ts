import type { Notice, Result, IUser, IRole } from '@/types/instance.resolver'
...
export async function fetchUserColumnr() {
    return await divineRequestCatcher<Result<IUser>>(
        await $fetch('/api/user/column', {
            headers: await useHeaders(),
            method: 'GET'
        })
    )
...
/**角色列表**/
export async function fetchRoleColumnr() {
    return await divineRequestCatcher<Result<IRole>>(
        await $fetch('/api/role/column', {
            headers: await useHeaders(),
            method: 'GET'
        })
    )
